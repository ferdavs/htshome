...
static const uint8_t READ_DATA_RETRY_COUNT = 60;
...
void PMSA003IComponent::setup() {
  ...
  if (!successful_read) {
    for (uint8_t i = 0; i < READ_DATA_RETRY_COUNT; i++) {
      delay(100);  // Give the sensor time to become ready
  ...
  if (!successful_read) {
    ESP_LOGW(TAG, "Initial read failed, sensor may need more warm-up time. Will retry during updates.");
    // Don't mark as failed - allow recovery during update()
    return;
  }
}
